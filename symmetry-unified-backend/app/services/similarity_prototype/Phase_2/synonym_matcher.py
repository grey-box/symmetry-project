...
class SynonymMatcher:
    ...
    def _normalize_word(self, word):
        normalized = self.preprocessor.remove_punctuation(word.lower())
        return normalized.strip()

    def _wordnet_pos_from_nltk_tag(self, tag):
        if tag.startswith("J"):
            return wordnet.ADJ
    ...
    def build_sentence_pos_cache(self, text):
        if not text:
            return
        ...
        for sentence in nltk.sent_tokenize(text):
            ...
            for raw_word, nltk_tag in tagged:
                normalized = self._normalize_word(raw_word)
                if not normalized:
                    continue
                if normalized in self._pos_cache:
                    continue
                self._pos_cache[normalized] = self._wordnet_pos_from_nltk_tag(nltk_tag)
    ...
    def get_pos_tag(self, word):
        ...
        if normalized in self._pos_cache:
            return self._pos_cache[normalized]

        tag = nltk.pos_tag([word])[0][1]

        # Convert nltk POS tags to wordnet POS tags
        result = self._wordnet_pos_from_nltk_tag(tag)
        self._pos_cache[normalized] = result
        return result
    ...
    def share_synset(self, word_a, word_b):
        ...
        synsets_a = {s.name() for s in wordnet.synsets(word_a)}
        synsets_b = {s.name() for s in wordnet.synsets(word_b)}
        ...
        for syn1 in synsets_1:
            for syn2 in synsets_2:
                ...
                if similarity is not None and similarity > max_similarity:
                    ...
                    if (
                        max_similarity >= 1.0
                    ):  # perfect score — no need to check more pairs
                        break
    ...
    def best_token_match(self, tokens_a, tokens_b):
        ...
        return round(
            total_similarity / len(tokens_a), 4
        )  # Average similarity across all tokens in A
    ...
    def compare(self, sentence_a, sentence_b):
        self.build_sentence_pos_cache(sentence_a)
        self.build_sentence_pos_cache(sentence_b)
