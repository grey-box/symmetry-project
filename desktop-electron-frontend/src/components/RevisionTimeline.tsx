...
export const computeRevisionSelection = (
    clickedRevid: number,
    selectedOld: number | null,
    selectedNew: number | null,
    sorted: Revision[],
): [number | null, number | null] => {
    if (selectedNew === clickedRevid) {
        return [selectedOld, null];
    }

    if (selectedOld === clickedRevid) {
        return [null, selectedNew];
    }

    if (selectedOld === null) {
        return [clickedRevid, selectedNew];
    }

    if (selectedNew === null) {
        const oldIdx = sorted.findIndex((r) => r.revid === selectedOld);
        const clickedIdx = sorted.findIndex((r) => r.revid === clickedRevid);
        if (clickedIdx > oldIdx) {
            return [selectedOld, clickedRevid];
        }
        return [clickedRevid, selectedOld];
    }

    const clickIdx = sorted.findIndex((r) => r.revid === clickedRevid);
    const oldIdx = sorted.findIndex((r) => r.revid === selectedOld);
    ...
    if (Math.abs(clickIdx - oldIdx) <= Math.abs(clickIdx - newIdx)) {
        ...
        return [selectedNew, clickedRevid];
    }
...
const RevisionTimeline: React.FC<RevisionTimelineProps> = ({
    ...
    const [selectedOldIndex, selectedNewIndex, selectedOldRevision, selectedNewRevision] = useMemo(() => {
        const oldIndex = selectedOld !== null ? sorted.findIndex((r) => r.revid === selectedOld) : -1;
        const newIndex = selectedNew !== null ? sorted.findIndex((r) => r.revid === selectedNew) : -1;

        return [
            oldIndex,
            newIndex,
            oldIndex >= 0 ? sorted[oldIndex] : undefined,
            newIndex >= 0 ? sorted[newIndex] : undefined,
        ] as const;
    }, [sorted, selectedOld, selectedNew]);

    const handleClick = useCallback(
        (revid: number) => {
            const [nextOld, nextNew] = computeRevisionSelection(revid, selectedOld, selectedNew, sorted);
            onChange(nextOld, nextNew);
        },
                ...
                <div className="flex items-start overflow-x-auto pb-2 gap-0">
                    {sorted.map((rev, idx) => {
                        ...
                        const isBetween =
                            selectedOldIndex !== -1 &&
                            selectedNewIndex !== -1 &&
                            idx > selectedOldIndex &&
                            idx < selectedNewIndex;
                ...
                <div className="flex gap-3 text-xs">
                    {selectedOld !== null && (
                        <span className="px-2 py-1 rounded bg-blue-50 text-blue-700 border border-blue-200">
                            A: rev {selectedOld}
                            {' — '}
                            {formatTimestamp(selectedOldRevision?.timestamp ?? '')}
                        </span>
                    )}
                    {selectedNew !== null && (
                        <span className="px-2 py-1 rounded bg-green-50 text-green-700 border border-green-200">
                            B: rev {selectedNew}
                            {' — '}
                            {formatTimestamp(selectedNewRevision?.timestamp ?? '')}
                        </span>
                    )}
                    {selectedOld !== null && selectedNew !== null && (
                        <button
